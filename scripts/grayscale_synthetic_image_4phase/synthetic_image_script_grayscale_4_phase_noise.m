...
%Add noise to image
%fnoise = add_noise2(f,0.03);
%fnoise = add_noise2(f,0.10);
fnoise = add_noise2(f, 0.05);

%with salt and pepper noise
%fnoise = imnoise(f,'salt & pepper', 0.01);

%rescale image
fg = rescale_image(fnoise);
[N,M] = size(fg);

%set parameters
pm.outer_iter = 20;
pm.alpha = 1.0;
pm.lambda =200; %for both gaussian and salt and pepper
pm.c = 1e-8;
pm.inner_iter = 300;
pm.tau = 1/4;
pm.sigma = 1/4;
